...
static struct offmode_charging_data htc_offmode_charging_data = {
	.duration_ms = 5000,
	.gpio_powerkey = BUZZ_GPIO_POWER_KEY,
	.reset_fnt = buzz_reset,
};

static struct platform_device buzz_offmode_charging_dev = {
	.name = "htc_offmode_charging",
	.id = -1,
	.dev	= {
		.platform_data = &htc_offmode_charging_data,
	},
};
...
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_HOME)  ":16:350:32:55"
	   ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":68:350:56:55"
	   ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":175:350:56:55"
	   ":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":224:350:32:55"
	   "\n");
}
